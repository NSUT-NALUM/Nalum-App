...
export class EventsService {
	...
	async create(input: EventCreateInput, actor: EventActor) {
		...
		if (notificationEmail) void enqueueEmail(
			"event-notification",
			{
				eventId: event.id,
	...
	async listPublished(filters: EventsQuery, viewerId: string) {
		...
		return { ...result, events: result.events.map((event) => this.toEvent(event)) };
	}
	...
	async get(eventId: string, actor: EventActor) {
		...
		if (!event || (event.status !== "PUBLISHED" && !this.canManage(actor, event.authorId))) {
	...
	async update(eventId: string, data: EventPatchInput, actor: EventActor) {
		const event = await this.requireManageable(eventId, actor, true);
		this.assertSchedule(data.startsAt ?? event.startsAt, data.endsAt ?? event.endsAt);
		return this.toEvent(await this.repository.update(eventId, data, actor.id));
	}
	...
	async join(eventId: string, userId: string) {
		...
		try {
			...
			if (this.isUniqueConstraint(error)) throw new EventRegistrationConflictError();
			throw error;
		}
	...
	async leave(eventId: string, userId: string) {
		...
		if (result.count !== 1) {
			throw new ConflictError("You have not joined this event", "EVENT_NOT_JOINED");
		}
		return { eventId, isJoined: false };
	}

	async listAttendees(eventId: string, actor: EventActor, filters: EventsQuery) {
		const event = await this.repository.findById(eventId, actor.id);
		if (!event) throw new EventNotFoundError();
		if (!this.canManage(actor, event.authorId)) throw new EventForbiddenError();
		return this.repository.listAttendees(eventId, filters.limit, filters.offset);
	}
	...
	async approve(eventId: string, actor: EventActor, note?: string) {
		if (actor.role !== "ADMIN") throw new EventForbiddenError();
		const result = await this.repository.moderate(eventId, actor.id, "PUBLISHED", note ?? null);
		if (result.count !== 1) throw new EventStateConflictError();
	...
	async reject(eventId: string, actor: EventActor, reason: string) {
		if (actor.role !== "ADMIN") throw new EventForbiddenError();
		const result = await this.repository.moderate(eventId, actor.id, "REJECTED", reason);
		if (result.count !== 1) throw new EventStateConflictError();
	...
	private async requireManageable(
		...
		if (event.authorId !== actor.id || (pendingOnlyForAuthor && event.status !== "PENDING")) {
	...
	private assertEventCreator(actor: EventActor) {
		if (actor.role !== "ADMIN" && (actor.role !== "ALUMNI" || actor.verificationStatus !== "VERIFIED")) {
			throw new EventForbiddenError();
		}
	}
	...
	private toEvent<T extends { imageKeys: string[]; registrations: unknown[]; _count: { registrations: number } }>(event: T) {
